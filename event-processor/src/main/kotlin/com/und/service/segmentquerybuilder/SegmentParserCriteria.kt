...
class SegmentParserCriteria {
    ...
    companion object {
        val logger: Logger = LoggerFactory.getLogger(SegmentParserCriteria::class.java)
    }
...
//    }
    ...
    private fun addDidNotAggregation(segment: Segment, listOfAggregation: MutableList<AggregationOperation>, tz: ZoneId, userIdentified: Boolean?) {
        ...
        didnot?.forEachIndexed { _, event ->
            listOfDidNotCriteria.add(parseEvents2(event, tz, false,userIdentified,segment.userId))
        }
...
//            var matchOperation = Aggregation.match(Criteria().norOperator(Criteria().orOperator(*listOfDidNotCriteria.toTypedArray())))
    ...
    private fun addDidAggregation(segment: Segment, listOfAggregation: MutableList<AggregationOperation>, tz: ZoneId, userIdentified: Boolean?) {
        ...
        did?.forEachIndexed { index, event ->
            var a = parseEvents1(event, tz, true,userIdentified,segment.userId)
    ...
    private fun addDidAggregationWithoutFacet(segment: Segment, listOfAggregation: MutableList<AggregationOperation>, tz: ZoneId, userIdentified: Boolean?){
        segment.didEvents?.let {
            ...
            did.forEachIndexed { _, event ->
                var matches = getListOfCriteria(event, tz,userIdentified,segment.userId)
    ...
    private fun getListOfCriteria(event: Event, tz: ZoneId, userIdentified:Boolean?,userId: String?):MutableList<Criteria>{
        ...
        userId?.let {
            matches.add(Criteria.where("userId").`is`(it))
        }
    ...
    private fun parseEvents1(event: Event, tz: ZoneId, did: Boolean, userIdentified: Boolean?,userId: String?): List<AggregationOperation> {

        var listOfAggregationOperation = mutableListOf<AggregationOperation>()
        var matches = mutableListOf<Criteria>()
        matches=getListOfCriteria(event,tz,userIdentified,userId)
    ...
    private fun parseEvents2(event: Event, tz: ZoneId, did: Boolean, userIdentified: Boolean?,userId: String?): Criteria {
