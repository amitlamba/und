...
import com.und.model.utils.SendGridConfig
...
class EmailSendService {
    ...
    fun sendEmailBySendGrid(sendGridConfig: SendGridConfig,email: Email){
        val from = SendGridEmail(email.fromEmailAddress?.address)
        val subject = email.emailSubject
        val to = SendGridEmail(email.toEmailAddresses[0].address)
        val content = SendGridContent("text/plain", email.emailBody)
        val mail = Mail(from, subject, to, content)
        val sg = SendGrid(sendGridConfig.sendGridApiKey)
        val request = Request()
        try {
            ...
            if(response.statusCode==400){
                logger.error("The email was not sent. Error message status code ${response.statusCode}")
            }
        } catch (e: IOException) {
            logger.error("The email was not sent. Error message: ${e.message}")
            val error = EmailError()
            with(error) {
                ...
                failedSettingId = email.clientEmailSettingId
                invalidAddresses = email.toEmailAddresses.toEmail()
                this.from = email.fromEmailAddress?.toString()
            }
            throw  EmailFailureException("Unable to send email : ${e.message}", e, error)
        }
    ...
    fun sendEmailBySMTP(emailSMTPConfig: EmailSMTPConfig, email: Email) {
        // Send the message.
        try {
            val session = emailHelperService.session(email.clientID, emailSMTPConfig,email.clientEmailSettingId?:-1)
